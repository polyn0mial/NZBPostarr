...
async def get_stream_monitors() -> Dict[str, Any]:
    ...
    return {"monitors": stream_monitors.list_stream_monitors()}
...
async def delete_stream_monitor(monitor_id: str) -> Dict[str, Any]:
    ...
    if not stream_monitors.remove_stream_monitor(monitor_id):
        raise HTTPException(status_code=404, detail="Stream monitor not found")
    await stream_monitors.restart_stream_monitors()
...
async def start_streamed_nzb_upload(
    ...
    try:
        request_options = stream_repost.normalize_stream_request(
    ...
    try:
        ...
        if file:
            ...
            resolved_category = stream_repost.resolve_stream_category(tmp_path, category)
        ...
        resolved_paths = stream_repost.resolve_source_nzb_paths(source_path)
        ...
        for path in resolved_paths:
            resolved_category = stream_repost.resolve_stream_category(path, category)
            job_ids.append(

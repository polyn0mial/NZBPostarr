...
def setup_config(total: int, tool_status: Dict[str, bool]) -> None:
    ...
    content = (
        "# NZBPostarr Configuration\n"
        f"# Generated by setup wizard on {_now_str()}\n"
        "# Edit this file or use the WebUI settings page.\n\n"
        + yaml.dump(config, default_flow_style=False, sort_keys=False, allow_unicode=True)
    )
    _write_config_atomically(content)

    ok(f"Config file written ({CONFIG_FILE})")


def _write_config_atomically(content: str) -> None:
    """Install the wizard's config through the validated atomic writer when it is importable.

    The app's writer validates the YAML against Config and restores the prior
    file on failure. Before the runtime is importable, fall back to a plain
    temp-file + os.replace so a crash never leaves a half-written config.
    """

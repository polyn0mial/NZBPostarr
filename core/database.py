...
# ==============================================================================
#  RELEASE NAME PARSING  (powered by guessit)
# ==============================================================================

_FALLBACK_TV_RE = re.compile(
    r"(?i)(?P<title>.+?)(?:[.\s_-]+)S(?P<season>\d{1,2})[.\s_-]*E(?P<episode>\d{1,3})(?:[.\s_-]*E?(?P<episode_end>\d{1,3}))?"
)
_FALLBACK_TVX_RE = re.compile(
    r"(?i)(?P<title>.+?)(?:[.\s_-]+)(?P<season>\d{1,2})x(?P<episode>\d{1,3})(?:-(?P<episode_end>\d{1,3}))?"
)
...
def parse_release_name(item_name: str) -> Dict[str, Any]:
    """Extract parsed_title, media_type, season_number, episode_number from a release name.

    Uses the *guessit* library for robust scene-name parsing instead of
    hand-rolled regexes.  The return dict is kept API-compatible with the
    ...
    # If the name contains a path separator (folder/file), parse the filename
    # part to avoid matching season markers in folder names like
    # "(2018) S01 1080p.../101.Dalmatian.Street.S01E01...mkv"
    if "/" in name:
    ...
    gtype = str(guess.get("type", "")).lower()  # "episode" or "movie"

    season = guess.get("season")
    episode = guess.get("episode")
    episode_end: int = 0

    # guessit can return a list for season in multi-season releases
    if isinstance(season, list):
        season = season[0] if season else None

    # guessit returns a list when there's a range (e.g. S01E03-E04)
    if isinstance(episode, list):
        ep_start = episode[0]
        ep_last = episode[-1]
        episode_end = ep_last if ep_last > ep_start else 0
        episode = ep_start
    else:
        ep_start = episode

    # Map guessit type → our media_type
    if gtype == "episode" or season is not None:
        media_type = "tv"
    elif gtype == "movie":
        media_type = "movie"
    else:
...
def get_stats_delta(hours: int = 1) -> Dict[str, float]:
    """Summary of performance over the last X hours."""
    try:
        with session_scope() as session:
            cutoff = datetime.now(timezone.utc) - timedelta(hours=hours)
            stmt = (
                select(func.sum(Upload.filesize), func.count(Upload.id))
                .join(UploadResult)
                .where(UploadResult.uploaded_at >= cutoff, UploadResult.status == "success")
            )
            res = session.execute(stmt).fetchone()
            if not res:
                return {"bytes": 0, "count": 0}
            return {"bytes": res[0] or 0, "count": res[1] or 0}
    except Exception as e:
        logger.error(f"Stats delta failed: {e}")
        return {"bytes": 0, "count": 0}

...
def test_dashboard_data_raises_typed_error_on_db_failure(monkeypatch) -> None:
    ...
    monkeypatch.setattr(db, "session_scope", lambda: _BrokenSession())

    with pytest.raises(DatabaseOperationalError):
        db.get_dashboard_data(["geek"])
...
def test_duplicate_and_history_queries_fail_closed_on_db_failure(monkeypatch) -> None:
    ...
    monkeypatch.setattr(db, "session_scope", lambda: _BrokenSession())
    operations = [
        lambda: db.check_duplicate_dynamic("release", "TV Episode", ["geek"]),
        lambda: db.get_duplicate_status_batch(["release"], ["geek"]),
        lambda: db.get_job_history(),
        lambda: db.delete_job_history(["job-id"]),
        lambda: db.get_recent_uploads(),
        lambda: db.get_grouped_uploads(),
        lambda: db.get_group_upload_items("title"),
...
def test_database_e2e_smoke() -> None:
    ...
    jobs = db.get_job_history(limit=10)
    ...
    db.record_nntp_success(key1, 123, "TV Episode")
    assert (
    ...
    recent = db.get_recent_uploads(limit=10)
    assert recent["total"] >= 1
    assert any(item.get("item_name") == key1 for item in recent["items"])

    grouped = db.get_grouped_uploads(page=1, per_page=10)
    ...
    job_uploads = db.get_uploads_for_job(job_id)
    ...
    assert (
        db.record_interface_stats(
            [{"name": "eth0", "upload": 1.0, "download": 2.0}],
        )
        is True
    )
    sys_hist = db.get_system_stats_history(limit=10)
    assert len(sys_hist.get("cpu", [])) >= 1
    iface_hist = db.get_interface_stats_history(limit=10)
    assert "eth0" in iface_hist
    db.prune_system_stats(max_records=1)

    # --- Aggregate/stat helpers ---
    hourly = db.get_hourly_upload_stats()
    ...
    assert db.mark_as_uploaded([key2], ["geek"], itype="Movie") == 1
    assert db.mark_as_uploaded([key2], ["geek"], itype="Movie") == 0

    assert db.delete_upload_item(key1) is True
    assert db.bulk_delete_upload_items([key2]) >= 1

    assert db.delete_job_history([job_id]) >= 1
...
def test_get_duplicate_status_batch_chunks_large_sqlite_expression() -> None:
    ...
    db.record_nntp_success(exact_key, 100, "TV Episode")
    assert (
        db.update_db_destination(
            dest="geek",
            _name=exact_key,
    ...
    db.record_nntp_success(suffix_key, 101, "TV Episode")
    assert (
        db.update_db_destination(
            dest="omg",
            _name=suffix_key,
    ...
    dupes = db.get_duplicate_status_batch(item_keys, ["geek", "omg"])
...
def test_database_e2e_dual_upload_race_does_not_crash() -> None:
    ...
    def _worker(dest_id: str) -> None:
        barrier.wait()
        db.record_nntp_success(key, 100, "TV Episode")
        ok = db.update_db_destination(
            dest=dest_id,
            _name=key,
            size=100,
            key=key,

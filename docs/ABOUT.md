...
- A muted known-issue signature has no expiry by design - MutedIssue is keyed only on (indexer_id, signature) with no timestamp/TTL field, so once muted it stays hidden from the History page forever until someone manually unmutes it, even after the underlying failure is fixed. anchors: `core/db/`
- Duplicate detection (check_duplicate_dynamic) is scoped per indexer_id, not global across the whole install - an item already posted to one indexer will still be offered and posted to a different configured indexer that has no record of it. anchors: `core/db/`
- There is no per-user account model - a single shared web password (enable_password/web_password) optionally gates the whole UI and API, and it is off by default on the assumption of a trusted network, so anyone who can reach the port has full control unless an operator turns auth on. anchors: `api/`
...
- `logic/` - Business logic in packages (jobs, pipeline, pending, classify, stream, stats, system): upload queue and job execution, pending-folder scan/classification, indexer uploaders, Usenet streaming/reposting, folder + stream monitors, stats collector, self-updater.
- `core/` - Shared foundation: Pydantic config model, SQLAlchemy models/queries (core/db/), the indexer YAML registry (core/indexers/) and submission builder, guessit-backed release-name parsing, misc utils.
...
- **Headless / CLI mode** - Every web-UI action (upload, queue control, stats, config, updates) is also a scriptable `--headless` subcommand with `--json` output for cron/automation. - `cli/commands/`, `cli/commands/`
- **Direct NZB reposting / Usenet-to-Usenet streaming** - Restreams an existing NZB's articles from one Usenet server straight to another without downloading to disk, from the CLI or a saved monitor. - `logic/stream/`, `cli/commands/`
- **Stream monitors** - Watches a folder for new .nzb files and automatically queues a repost job for each one found, for long-running unattended reposting. - `logic/stream/`, `logic/stream/`
- **Folder monitor** - Watches configured content folders and auto-triggers an upload once a new item settles (stops changing size), no manual scan needed. - `logic/autoupload.py`, `logic/autoupload.py`
- **Upload queue with pause/resume/priority** - Concurrent upload queue with pause/resume, priority promotion, job retry, and persistence across restarts. - `logic/jobs/`, `logic/jobs/`, `logic/jobs/`
- **Duplicate detection** - Checks a candidate item against past uploads per indexer and skips content already posted there. - `core/db/`
- **Upload history & stats** - Browsable, filterable/grouped history of every completed upload with per-indexer results, plus hourly and detailed stat charts backed by SQLite. - `core/db/`, `core/db/`
- **Live system & network stats** - Live CPU/memory/network/disk and per-process resource panel with historical charts, viewable in the UI or via `--headless stats --watch`. - `logic/stats/collector.py`, `cli/commands/`
...
- **Automatic content classification** - Infers category (TV/movie/anime/disc/music/ebook/etc.) and season/episode structure from folder and file names via guessit plus custom heuristics, with a manual override path per item. - `logic/pending/tree.py`, `core/release_name.py`
- **Cross-platform daemon mode** - Runs as a background daemon (start/status/stop) on Windows, macOS, and Linux, with an optional persistent tmux session and a wizard-generated systemd unit for production. - `cli/launcher/`, `cli/launcher/`, `setup.py`
- **Auto-bootstrap installer** - Running `python3 main.py` creates its own virtualenv and installs pinned dependencies on first run - no manual setup step. - `cli/launcher/`
- **First-run setup wizard** - Interactive guided installer configures content folders, NNTP servers, and indexer API keys on first launch when no config file is present. - `setup.py`, `setup.py`
- **Process reaper** - Periodically finds and kills orphaned nyuu/rar/parpar child processes left behind by crashed or interrupted jobs. - `logic/system/reaper.py`, `logic/system/reaper.py`
- **Self-update / rollback** - Checks GitHub releases for newer versions, downloads and applies them in place while protecting runtime data, with rollback to any previous backup snapshot. - `logic/system/updater.py`, `logic/system/updater.py`
- **Login / session auth** - Optional shared web password gates the UI and API behind a signed session cookie; off by default for trusted-network deployments. - `api/`, `api/`
- **Settings & raw config editor** - Edit all settings (folders, servers, indexers, security) through UI forms, or view/edit the raw YAML directly, with secrets masked in every API response. - `api/`, `api/`
...
- **Known issues view** - A collapsible Known Issues panel on the History page groups failed indexer submissions by (indexer, normalized error signature), showing occurrence count, affected-item count, a sample message, and first/last-seen timestamps for each signature. Each issue can be muted or unmuted, backed by a new MutedIssue model keyed on (indexer_id, signature), so a noisy known failure can be quieted from the default view without losing its upload history. Exposed via GET /api/uploads/errors/grouped, POST /api/uploads/errors/mute and /errors/unmute, and `--headless issues`. - `core/db/`, `core/db/`, `core/db/`, `api/history.py`, `webui/history.html`, `webui/assets/js/pages/history.js:665`

**Partial - exists but incomplete, gated off, or known broken**

- **MCP endpoint for AI agents** - Exposes read-only (and, if explicitly enabled, job-control) tools over MCP at /mcp with its own bearer token; off by default and needs a manual `pip install mcp` since the SDK isn't bundled. - `api/mcp.py`, `api/mcp.py`
...
- **a new HTTP API route** - add a handler to the relevant APIRouter in app.py (uploads_router, pending_router, indexers_router, etc.). anchors: `api/`
- **a new headless CLI command** - add a cmd_* function in logic/headless.py and wire it into build_headless_parser's subparsers. anchors: `cli/commands/`
- **a new WebUI page** - add a template under webui/ plus a page controller under webui/assets/js/pages/, extending page-base.js; serve it via app.py's get_page route. anchors: `api/`, `webui/assets/js/page-base.js:1`
- **a new DB-backed record type** - add a SQLAlchemy model in core/database.py beside Upload/JobHistory/SystemStat and a matching query helper. anchors: `core/db/`
- **a new content-classification rule** - extend the heuristics in logic/pending_scan.py (classify_video_name_result and friends) or the guessit fallback in core/release_name.py. anchors: `logic/pending/tree.py`

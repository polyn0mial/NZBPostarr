...
- Upgrades across layouts: the in-app updater of installs older than 9.5 (the nested `nzbpostarr/` package) only accepts an archive containing `nzbpostarr/app.py`, so it refuses 9.6+ and changes nothing; those installs move by fresh install. A 9.5.x install updated in place keeps its removed files (harmless; listed in logic/system/removed_paths.txt) and reinstalls dependencies on first start because requirements.lock changed. anchors: `tests/test_release.py`, `cli/launcher/bootstrap.py`
